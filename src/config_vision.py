...
class TableHomography:
    ...
    2026-08-19 重新標定（相機又調整過角度，08-18 那組已失效），
    ...
    以上像素值取自 座標/座標.txt（test/yolo_camera_test.py 的原始輸出），
    不是從試算表轉抄的——試算表把 (-200,200) 誤抄成 103,70，實際是 93,70。

    ⚠️ 殘差 RMS 4.88mm、最大 8.38mm——比 08-13 的 2.13mm、08-18 的 2.93mm
       都差，原因是 x=-100 那一欄的量測散得比較開，不是模型問題：

           同一 x 欄的 u 值全距    x=0 → 6px、x=-100 → 15px、x=-200 → 10px
           最差的兩點              (-100,100) 差 8.38mm、(-100,200) 差 7.01mm

       前兩次標定 RMS 都在 2~3mm，代表這個方法可以更準，這次是量測本身
       偏掉了。
       → 待辦：重量 x=-100 那三點，重量後 RMS 應可回到 2~3mm。
       夾食材時 5mm 與 8mm 偏差有感（小黃瓜直徑約 30mm）。

    ⚠️ 這次相機已接近垂直於檯面：仿射殘差 RMS 5.28mm、單應性 4.88mm，只
       差 8%（08-13 差 51%、08-18 差 16%）。透視變形大幅減少，但仍維持
       單應性——它涵蓋仿射，沒有換回去的理由。
    """
    ...
    H = np.array([
        [+1.17141972e+00, -3.35332435e-02, -2.99879666e+02],
        [-2.52710928e-02, -1.24687757e+00, +2.82655504e+02],
        [-2.59237353e-04, -2.04378738e-04, +1.00000000e+00],
    ], dtype=np.float64)
