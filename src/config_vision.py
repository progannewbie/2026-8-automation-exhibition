...
class TableHomography:
    ...
    2026-09-02 重新標定（相機當天調整兩次，08-19 那組已失效），
    3×3 網格 9 點（手動移動手臂到位後讀 YOLO 中心點）。
    來源：座標/手臂座標跟視覺座標 (1) (1).xlsx
    ...
    ⚠️ 網格範圍換過：x 從「0 ~ -200」改成「+100 ~ -100」。標定只在取樣範圍
       內是內插，所以現在涵蓋的檯面區域跟先前不同，取料區若落在 x < -100
       就是外推。
    ...
    ⚠️ 殘差 RMS 3.81mm、最大 7.91mm。歷次：

           08-13  2.13 / 3.19
           08-18  2.93 / 4.97
           08-19  4.88 / 8.38   x=-100 那欄量測偏掉
           09-02  5.60 / 9.39   (100,0) 離群，留一驗證 21.3mm，未採用
           09-02  3.81 / 7.91   ← 本組，重測後無離群點

       這組資料本身一致：各列每 100mm 的像素距離差異都在 8% 以內（前一版
       y=0 那列差 35%），留一交叉驗證最大 12.31mm、平均 5.68mm，沒有單點
       特別離群。殘差與位置無相關性（相關係數皆 <0.05），代表剩下的是
       「手動移動手臂 + 像素讀整數」的固有誤差，換模型沒有用。

       唯一稍大的是 (0,100)=230,290 差 7.91mm——x=0 那欄的 u 是 235/230/234，
       中間偏小 4~5px。影響有限，未特別處理。

    ⚠️ 仿射殘差 RMS 4.27mm、單應性 3.81mm，差 11%。透視變形已不明顯（相機
       接近垂直），但仍維持單應性——它涵蓋仿射，沒有換回去的理由。
    """
    ...
    H = np.array([
        [+1.10603259e+00, -3.13822329e-02, -2.48917447e+02],
        [-2.98918212e-02, -1.20349336e+00, +4.41604018e+02],
        [+2.08197084e-05, -2.91353539e-04, +1.00000000e+00],
    ], dtype=np.float64)
