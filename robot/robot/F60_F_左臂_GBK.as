...
.PROGRAM SYNC_STEP(.ok)
  ...
  .ok = ok1
.END
.PROGRAM DO_PICKUP(.$location,.$arm,.x_mm,.y_mm,.angle_deg) #8
  IF .$arm <> "F60_F" THEN
...
.PROGRAM DO_PICKUP_test()
  ...
  LMOVE chop_depart_pt
.END
.PROGRAM DO_CHOP_test (.$food,.cuts,.thick) ; 
  ...
  LMOVE #WORK_CHOP_ZONE
  FOR .t=0 TO 30
    POINT chop_per[.t] = SHIFT(chop_ORIGIN BY (.t-1)*5 ,-20,50)
    POINT chop_1[.t] = SHIFT(chop_ORIGIN BY (.t-1)*5 ,0,0)
  END
  LMOVE chop_per[7]
  LMOVE chop_1[7]
  LMOVE chop_per[7]
